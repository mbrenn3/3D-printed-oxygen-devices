...
\begin{flushleft}
{\Large
\textbf\newline{3D-Printed 24-Well Plate Oxygen Control Device - Submission to PLOS ONE}
}
\newline
...
\section*{Abstract}
Recently 3D printing has emerged as a method for directly printing complete microfluidic devices, although printing materials have been limited to non-oxygen permeable materials.
...
We demonstrate enhanced transcription of VEGFA (vascular endothelial growth factor A) gene with decreasing oxygen levels in human lung adenocarcinoma cells.
...
Studying cancer cells under controlled hypoxic conditions is important in understanding the pathophysiology because research has shown hypoxia may enhance aggressive phenotypes, tumor progression, metastasis, and resistance to therapy.
...
Sensors were prepared as discussed previously \cite{Sinkala2010}. 
Briefly, polystyrene is dissolved in toluene to form a solution to which PtOEPK is mixed in before spinning coating to a thin layer and allowing the toluene to evaporate.
The intensity of the sensor was measured with via fluorescence microscopy and correlated to the concentration of oxygen with Stern-Volmer analysis (Fig.~\ref{fig2}).
...
The inlet tubing was placed in an open cone that was flooded with gas from a tank with a flow that exceed the vacuum flow so that the gas of interest is pulled into the inlet rather than room air due to the open connection.
...
All gas tanks used in the oxygen characterization were 5\% \ce{CO2} in addition to the desired gas mix in anticipation of cell culture experiments as \ce{CO2} can alter the fluorescence of the PtOEPK.
The this entire procedure was repeated three times for a sample size of three. 
...
\begin{figure}[h]
\caption{
{\bf PCR Data.}  VEGF expression in A549 cells after exposure to different oxygen conditions set-up by the insert device. 
}
\label{fig3}
\end{figure}
...
A different gas condition can be used in each separate iteration of the 6-well unit allowing 6 technical replicates of up to 4 different gas conditions in one 24 well plate (Fig.~\ref{fig2}). There was little variation in the oxygen levels measured in each of the 6 well controlled by a single inlet, demonstrating the distribution network worked effectively.
...
When a HIF transcription factor undergoes nuclear translocation, it can initiate transcription of genes by binding to the hypoxia response element (HRE) on its target genes.
VEGFA has previously been shown to have a functional HRE, and upregulation has been observed in A549 cells under hypoxic conditions.
...
Oxygen control is demonstrated in a 24 well plate and PCR detection of upregulation of VEGFA mRNA in the A549 shows the device effectively controls oxygen as expected. 
